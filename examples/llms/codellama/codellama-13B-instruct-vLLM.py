...
class CodeLlamaPipeline:
    ...
    def load_model(self) -> None:
        ...
        model_dir = Path("~/.cache/huggingface/codellama/CodeLlama-13b-instruct-hf").expanduser()
...
if local:
    ...
    upload = pipelines.upload_pipeline(my_pl, "CodeLlama-13b-instruct:latest", "codellama_w_vllm", required_gpu_vram_mb=30_000, accelerators = [compute_requirements.Accelerator.nvidia_a100_80gb])

    # Remote run
    start = time.time()
    output = pipelines.run_pipeline("CodeLlama-13b-instruct:latest", *model_inputs)
    end = time.time() - start
